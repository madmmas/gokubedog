...
var _ = Describe("PolicyProfile Controller", func() {
	Context("When reconciling a resource", func() {
		...
		policyprofile := &watchdogv1alpha1.PolicyProfile{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind PolicyProfile")
			err := k8sClient.Get(ctx, typeNamespacedName, policyprofile)
			if err != nil && errors.IsNotFound(err) {
				resource := &watchdogv1alpha1.PolicyProfile{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance PolicyProfile")
